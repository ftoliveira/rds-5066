...
#
# Diferente do chat_app_sis (UDP ponto-a-ponto), aqui cada nó é um DTE que abre
# uma conexão TCP com um MODEM MIL-STD-188-110D (Appendix A): o `rds-hf` real ou
# o modem-mock local (`mock_110d_air.py`). A ponte OTA entre os dois nós é feita
# pelo modem — o app só conhece host:porta do seu modem. A taxa efetiva é
# reportada pelo modem (Transmit Setup); os seletores abaixo afinam o ARQ local.
# ---------------------------------------------------------------------------
SUPPORTED_BITRATES = [150, 300, 600, 1200, 2400]
SUPPORTED_INTERLEAVERS = ["short", "long"]

# Portas TCP default do modem por nó (casam com o lançador mock_110d_air.py:
# nó A → 3000, nó B → 3001). Para um rds-hf real, ajuste host/porta na UI.
DEFAULT_MODEM_PORT_A = 3000
DEFAULT_MODEM_PORT_B = 3001
...
class ChatApp:
    def __init__(self, root: tk.Tk, node_name: str):
        ...
        # Alvo do modem 110D (DCE) que este DTE conecta via TCP. Default casa com
        # o lançador mock local; para um rds-hf real, edite os campos na UI.
        self.modem_host = "127.0.0.1"
        self.modem_port = DEFAULT_MODEM_PORT_A if self.is_node_a else DEFAULT_MODEM_PORT_B
        # SAP 0 é reservado ao Subnet Management (requer allow_management_rank);
        # usamos SAPs de dados de usuário comuns para o chat.
        self.bound_saps = (3, 5)
        self._prev_modem_connected: bool | None = None

        self.root.title(f"STANAG 5066 HF Chat (110D/TCP) — Node {self.node_name}")
    ...
    def _build_ui(self) -> None:
        ...
        # ── Linha 2: alvo do modem 110D (host:porta) + status da conexão ──
        ttk.Label(cfg_frame, text="Modem host").grid(
            row=1, column=0, padx=(0, 4), pady=(6, 0), sticky="w",
        )
        self.ent_modem_host = ttk.Entry(cfg_frame, width=14)
        self.ent_modem_host.grid(row=1, column=1, padx=(0, 12), pady=(6, 0), sticky="w")
        self.ent_modem_host.insert(0, self.modem_host)

        ttk.Label(cfg_frame, text="Porta").grid(
            row=1, column=2, padx=(0, 4), pady=(6, 0), sticky="w",
        )
        self.ent_modem_port = ttk.Entry(cfg_frame, width=8)
        self.ent_modem_port.grid(row=1, column=3, padx=(0, 12), pady=(6, 0), sticky="w")
        self.ent_modem_port.insert(0, str(self.modem_port))

        self.lbl_modem_status = ttk.Label(
            cfg_frame, text="Modem: —", foreground="gray", style="Status.TLabel",
        )
        self.lbl_modem_status.grid(row=1, column=4, columnspan=2, padx=(8, 0), pady=(6, 0), sticky="w")

        self.lbl_modem_rate = ttk.Label(
            cfg_frame, text="Taxa modem: —", foreground="gray", style="Status.TLabel",
        )
        self.lbl_modem_rate.grid(row=1, column=6, padx=(12, 0), pady=(6, 0), sticky="w")
    ...
    def _cmd_init_node(self) -> None:
        ...
        self.ent_modem_host.config(state="disabled")
        self.ent_modem_port.config(state="disabled")

        try:
            self._init_stanag(bitrate, interleaver)
            self.lbl_init_status.config(text="⬤ Online", foreground="#4CAF50")
            # Conectar/Desconectar (hard link CAS) só liberam após o modem conectar.
            self._log_msg(
                "SYSTEM",
                f"Nó inicializado (Fase 4/SIS). Modem 110D alvo "
                f"{self.modem_host}:{self.modem_port}, SAPs:{self.bound_saps}, "
                f"bitrate={bitrate}, interleaver={interleaver}. "
                f"Aguardando handshake Appendix A...",
                "system",
            )
            self._start_sis_api()
            ...
            self.ent_modem_host.config(state="normal")
            self.ent_modem_port.config(state="normal")

    def _init_stanag(self, bitrate: int, interleaver: str) -> None:
        # Lê o alvo do modem 110D dos campos da UI (host:porta).
        self.modem_host = self.ent_modem_host.get().strip() or "127.0.0.1"
        try:
            self.modem_port = int(self.ent_modem_port.get().strip())
        except ValueError:
            self.modem_port = DEFAULT_MODEM_PORT_A if self.is_node_a else DEFAULT_MODEM_PORT_B

        # DTE 110D: conecta via TCP ao modem (Appendix A). O adaptador faz o
        # handshake, mantém keep-alive e reconecta sozinho; data_rate_bps passa
        # a refletir o Transmit Setup reportado pelo modem.
        adapter = Tcp110dModemAdapter(
            Tcp110dConfig(
                host=self.modem_host,
    ...
    def _update_status_labels(self) -> None:
        ...
        # ── Estado da conexão TCP com o modem 110D (DCE) ──
        modem = node.modem
        connected = getattr(modem, "is_connected", False)
        if connected:
            self.lbl_modem_status.config(text="Modem: ⬤ Conectado", foreground="#4CAF50")
            rate = getattr(modem, "reported_data_rate", node.modem.config.data_rate_bps)
            blk = getattr(modem, "reported_blocking_factor", 0)
            self.lbl_modem_rate.config(
                text=f"Taxa modem: {rate} bps | blk {blk}", foreground="#4CAF50",
            )
        else:
            self.lbl_modem_status.config(text="Modem: ⬤ Conectando…", foreground="#FF9800")
            self.lbl_modem_rate.config(text="Taxa modem: —", foreground="gray")
        # Log de transição de conexão do modem
        if self._prev_modem_connected != connected:
            self._prev_modem_connected = connected
            if connected:
                self._log_msg("SYSTEM", f"Modem 110D conectado ({self.modem_host}:{self.modem_port}) — handshake Appendix A OK.", "system")
            else:
                self._log_msg("SYSTEM", "Modem 110D desconectado — adaptador tentará reconectar.", "error")
        ...
        # Button state — Conectar (hard link CAS) só faz sentido com o modem 110D
        # conectado, pois os frames precisam do enlace TCP com o DCE.
        if cas_state == CasLinkState.IDLE:
            self.btn_connect.config(state="normal" if connected else "disabled")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "STANAG 5066 HF Chat App (Fase 4 SIS) sobre modem MIL-STD-188-110D "
            "(Appendix A / TCP). Cada nó é um DTE que conecta a um modem (rds-hf "
            "real ou o mock local mock_110d_air.py). Defaults de porta: A→3000, B→3001."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--modem-host", default=None,
        help="Host do modem 110D (default 127.0.0.1; sobrescreve o campo da UI)",
    )
    parser.add_argument(
        "--modem-port", type=int, default=None,
        help="Porta TCP do modem 110D (default A=3000 / B=3001)",
    )
    args = parser.parse_args()

    root = tk.Tk()
    app = ChatApp(root, args.node)
    if args.modem_host is not None:
        app.modem_host = args.modem_host
        app.ent_modem_host.delete(0, tk.END)
        app.ent_modem_host.insert(0, args.modem_host)
    if args.modem_port is not None:
        app.modem_port = args.modem_port
        app.ent_modem_port.delete(0, tk.END)
        app.ent_modem_port.insert(0, str(args.modem_port))
